...
class NinjaGestureRecognizer:
    ...
    def __init__(self, osc_ip=None, osc_port=None, stabilizer_settings=None):
        ...
        logger.info(f"Ninja Gesture Recognizer (3 Gestures) initialized - OSC: {osc_ip or Config.OSC_IP}:{osc_port or Config.OSC_PORT}")
    ...
    def send_gesture_osc(self, gesture_type_enum, gesture_data, hand_label):
        ...
        try:
            ...
            logger.info(f"OSC Sent: {gesture_type_str} ({action}) - {hand_label}, Conf: {confidence:.2f}")
            ...
            if results.multi_hand_landmarks:
                for hand_idx, hand_landmarks_obj in enumerate(results.multi_hand_landmarks):
                    ...
                    if should_send and raw_gesture_type != GestureType.NONE:
                        ...
                        debug_messages_for_frame.append(f"{hand_label}: {raw_gesture_type.value} ({action}) ✓")
                    else:
...
class NinjaMasterHandTracker:
    ...
    def __init__(self, osc_ip=None, osc_port=None, stabilizer_settings_override=None):
        ...
        logger.info("Ninja Master Hand Tracker (3 Gestures) initialized.")
    ...
    def draw_debug_info(self, frame, fps, debug_messages_list):
        ...
        # 제스처 가이드 - 3개만
        guide_y = Config.CAMERA_HEIGHT - 140
        cv2.putText(frame, "=== 3 Core Gestures ===", (10, guide_y), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2, cv2.LINE_AA)
        
        gestures = [
            ("1. FLICK", "Quick finger movement", "Throw Shuriken", (255, 100, 100)),
            ("2. FIST", "Close your hand", "Block Attack", (100, 255, 100)),
            ("3. PALM PUSH", "Open all 5 fingers", "Shock Wave", (100, 100, 255))
        ]
        ...
        # 하단 정보
        cv2.putText(frame, "Ninja Master - Simple 3 Gesture System", (10, Config.CAMERA_HEIGHT - 40), 
...
def test_mode():
    ...
    try:
        ...
        logger.info("=== OSC 테스트 모드 (3 Gestures) ===")
        tester = OSCTester()
        tester.start_server()
...
if __name__ == "__main__":
    ...
    if is_test_mode:
        ...
        print("\n")
        print("=" * 50)
        print("    닌자 마스터 - 3 제스처 시스템")
        print("=" * 50)
        print("\n핵심 제스처:")
        print("  1. FLICK     - 손가락 튕기기 → 표창 던지기")
        print("  2. FIST      - 주먹 쥐기     → 공격 막기")
        print("  3. PALM PUSH - 5손가락 펴기  → 진동파")
        print("\n조작법:")
        print("  • Q - 종료")
        print("  • D - 디버그 모드 전환")
        print("=" * 50)
